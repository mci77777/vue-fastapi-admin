...
-- =====================================================
-- 创建日期: 2025-09-29
-- 目标: 验证服务器端与Android Room数据库架构对齐情况
...
BEGIN
    ...
    LOOP
        IF EXISTS (
            ...
            RAISE NOTICE '✅ 表 % 存在', current_table;
        ELSE
            missing_tables := array_append(missing_tables, current_table);
            RAISE NOTICE '❌ 表 % 缺失', current_table;
        END IF;
    END LOOP;
...
BEGIN
    ...
    LOOP
        IF EXISTS (
            ...
            AND c.column_name = current_column
        ) THEN
            existing_count := existing_count + 1;
            RAISE NOTICE '✅ 字段 users.% 存在', current_column;
        ELSE
            missing_columns := array_append(missing_columns, current_column);
            RAISE NOTICE '❌ 字段 users.% 缺失', current_column;
        END IF;
    END LOOP;
...
BEGIN
    ...
    LOOP
        ...
        IF table_pk_info.is_correct THEN
            correct_count := correct_count + 1;
            RAISE NOTICE '✅ %.% 主键类型正确: %',
                table_pk_info.table_name, table_pk_info.column_name, table_pk_info.data_type;
        ELSE
...
BEGIN
    ...
    RAISE NOTICE '   - 验证通过: %', correct_count;
END $$;
...
BEGIN
    ...
    FOR index_info IN
        ...
        AND tablename IN ('users', 'user_profiles', 'user_settings', 'calendar_events',
                         'chat_sessions', 'chat_raw', 'message_embedding', 'session_summary')
        AND indexname NOT LIKE '%_pkey'
        ORDER BY tablename, indexname
    LOOP
        index_count := index_count + 1;
        RAISE NOTICE '✅ 索引: %.%', index_info.tablename, index_info.indexname;
    END LOOP;

    RAISE NOTICE '';
    RAISE NOTICE '📊 索引验证结果:';
    RAISE NOTICE '   - 性能索引数量: %', index_count;
END $$;
...
BEGIN
    ...
    FOR policy_info IN
        ...
        ORDER BY tablename
    LOOP
        IF policy_info.rowsecurity THEN
            rls_enabled_count := rls_enabled_count + 1;
            RAISE NOTICE '✅ RLS已启用: %', policy_info.tablename;
        ELSE
            RAISE NOTICE '❌ RLS未启用: %', policy_info.tablename;
        END IF;
    END LOOP;
...
BEGIN
    ...
    FOR type_info IN
        SELECT
            ...
            c.data_type,
            CASE
                WHEN c.column_name LIKE '%Id' AND c.data_type = 'uuid' THEN true
                WHEN c.column_name LIKE 'is%' AND c.data_type = 'integer' THEN true
                WHEN c.column_name LIKE '%At' AND c.data_type = 'bigint' THEN true
                WHEN c.column_name = 'weight' AND c.data_type = 'real' THEN true
                WHEN c.column_name LIKE '%Json' AND c.data_type = 'jsonb' THEN true
                WHEN c.data_type IN ('text', 'integer', 'bigint', 'real', 'bytea') THEN true
                ELSE false
    ...
    LOOP
        total_count := total_count + 1;
        IF type_info.is_compatible THEN
            compatible_count := compatible_count + 1;
            RAISE NOTICE '✅ %.% 类型兼容: %',
                type_info.table_name, type_info.column_name, type_info.data_type;
        ELSE
            RAISE NOTICE '⚠️  %.% 类型需检查: %',
                type_info.table_name, type_info.column_name, type_info.data_type;
        END IF;
    END LOOP;

    RAISE NOTICE '';
    RAISE NOTICE '📊 数据类型兼容性验证结果:';
    RAISE NOTICE '   - 检查的字段: %', total_count;
    RAISE NOTICE '   - 兼容字段: %', compatible_count;
    RAISE NOTICE '   - 需要检查: %', total_count - compatible_count;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '📊 统计信息:';
    RAISE NOTICE '   - 核心表数量: % / 13', table_count;
    RAISE NOTICE '   - 性能索引: %', index_count;
    RAISE NOTICE '   - RLS策略: %', policy_count;
